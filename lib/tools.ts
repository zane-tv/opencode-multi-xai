...
import { resolveAccount, shortId } from "./tools/resolve.js";
import { renderStatusLine } from "./tui-status.js";

/**
 * CLI management tools registered via the plugin `tool` hook.
 *
 * This lives in its OWN module (not lib/plugin.ts) on purpose: OpenCode's
 * plugin loader falls back to iterating EVERY export of the plugin entry file
 * and invoking each function-valued export as a `Plugin`. A non-plugin function
 * export in the entry file (like `buildTools`, whose signature is
 * `(manager) => ...`, not `(input) => Promise<Hooks>`) would be called with a
 * `PluginInput`, throw, and cause the whole plugin to be silently dropped — so
 * its OAuth login methods never appear in `opencode auth login`. Keeping this
 * out of the entry file guarantees the entry only default-exports `{ id, server }`.
 *
 * Every tool operates on the shared AccountManager and mutates ONLY through its
 * methods (never storage directly). NO tool ever prints a refresh or access
 * token value — outputs are limited to account ids, labels, and state.
 */
...
export function buildTools(
  ...
  return {
    ...
    "xai-prune": tool({
      ...
      async execute(args) {
        // Safe default: destructive delete requires an explicit dryRun=false.
